...
def measure_fn(measure: str, input_tensor: Tensor, *args, **kwargs) -> Float[Tensor, "..."]:
    avail_measures = {"mean": torch.mean, "median": torch.median, "max": torch.max, "stack": torch.stack}

    try:
        return avail_measures[measure](input_tensor, *args, **kwargs)
    except KeyError:
        raise NotImplementedError(
            f"Unknown measure function '{measure}'. Available measures:" + ", ".join([f"'{str(fn)}'" for fn in avail_measures.keys()])
        )
...
class ModelAbliterator:
    ...
    def calculate_mean_dirs(self, key: str, include_overall_mean: bool = False) -> Dict[str, Float[Tensor, "d_model"]]:
        dirs = {"harmful_mean": torch.mean(self.harmful[key], dim=0), "harmless_mean": torch.mean(self.harmless[key], dim=0)}
    ...
    def refusal_dirs(self, invert: bool = False) -> Dict[str, Float[Tensor, "d_model"]]:
        ...
        refusal_dirs = {
            key: self.calculate_mean_dirs(key) for key in self.harmful if ".0." not in key
    ...
    def tokenize_instructions_fn(self, instructions: List[str]) -> Int[Tensor, "batch_size seq_len"]:
    ...
    def generate(
        self, prompt: List[str] | str, *model_args, max_tokens_generated: int = 64, stop_at_eos: bool = True, **model_kwargs
    ) -> List[str]:
        ...
        logits, all_toks = self.generate_logits(
            gen, *model_args, stop_at_eos=stop_at_eos, max_tokens_generated=max_tokens_generated, **model_kwargs
        )
        return self.model.tokenizer.batch_decode(all_toks, skip_special_tokens=True)

    def test(self, *args, test_set: List[str] = None, N: int = 16, batch_size: int = 4, **kwargs):
        if test_set is None:
    ...
    def run_with_cache(
        ...
        **model_kwargs,
        ...
        cache_dict, fwd, bwd = self.model.get_caching_hooks(
            names_filter, incl_bwd, device, remove_batch_dim=remove_batch_dim, pos_slice=utils.Slice(None)
        )
    ...
    def apply_refusal_dirs(
        self, refusal_dirs: List[Float[Tensor, "d_model"]], W_O: bool = True, mlp: bool = True, layers: List[str] = None
    ):
        if layers == None:
    ...
    def induce_refusal_dir(self, refusal_dir: Float[Tensor, "d_model"], W_O: bool = True, mlp: bool = True, layers: List[str] = None):
    ...
    def find_best_refusal_dir(
        self, N: int = 4, positive: bool = False, use_hooks: bool = True, invert: bool = False
        ...
        for direction in tqdm(dirs.items()):
            score = self.test_dir(direction[1], N=N, use_hooks=use_hooks)
            if type(score) == dict:
                score = score["positive" if positive else "negative"]
            else:
                score = score[int(positive)]
            scores.append((score, direction))
        return sorted(scores, key=lambda x: x[0])

    def measure_scores(
        self, N: int = 4, sampled_token_ct: int = 8, measure: str = "max", batch_measure: str = "max", positive: bool = False
    ) -> Dict[str, Float[Tensor, "d_model"]]:
    ...
    def measure_scores_from_logits(
        self, logits: Float[Tensor, "batch_size seq_len d_vocab"], sequence: int, measure: str = "max"
    ...
    def mse_positive(self, N: int = 128, batch_size: int = 8, last_indices: int = 1) -> Dict[str, Float[Tensor, "d_model"]]:
    ...
    def create_activation_cache(
        self, toks, N: int = 128, batch_size: int = 8, last_indices: int = 1, measure_refusal: int = 0, stop_at_layer: int = None
        ...
        for i in tqdm(range(0, min(N, len(toks)), batch_size)):
            logits, cache = self.run_with_cache(
                toks[i : min(i + batch_size, len(toks))], max_new_tokens=measure_refusal, stop_at_layer=stop_at_layer
            )
    ...
    def cache_activations(
        ...
        self.harmful, self.harmful_z_label = self.create_activation_cache(
            harmful_toks, N=N, batch_size=batch_size, last_indices=last_indices, measure_refusal=measure_refusal, stop_at_layer=None
        )
        if not preserve_harmless:
            self.harmless, self.harmless_z_label = self.create_activation_cache(
                harmless_toks, N=N, batch_size=batch_size, last_indices=last_indices, measure_refusal=measure_refusal, stop_at_layer=None
            )
